...
import assests, { messagesDummyData } from "../assets/assets";
  ...
  return selectedUser ? (
    <div className="h-full overflow-scroll relative backdrop-blur-lg">
      ...
      <div className="flex items-center gap-3 py-3 mx-4 border-b border-stone-500">
        <img src={assests.profile_martin} alt="" className="w-8 rounded-full" />
  ...
  <p className='text-gray-500'>{ formatMessageTime(msg.createdAt)}</p>
    ...
    <div className="absolute  bottom-0  left-0 right-0 flex items-center gap-3 p-3">
      <div className="flex-1 flex items-center bg-gray-100/12 px-3 rounded-full">
        <input type="text" placeholder="Send a message" className="flex-1 text-sm text-white p-3 rounded-lg outline-none border-none placeholder-gray-400" />
        <input type="file" id="image" accept="image/png, image/jpeg" hidden/>
        <label htmlFor="image">
          <img src={assests.gallery_icon} alt="" className="w-5 mr-2 cursor-pointer"/>
        </label>
      </div>
      <img src={assests.send_button} alt="" className="w-7 cursor-pointer"/>
    </div>
    </div>
  ) : (
